...
package object structural extends DebugEnhancedLogging {
  def containsDir(d: Path)(t: TargetBag): Try[Unit] = Try {
    ...
    if (!(t.bagDir / d.toString).isDirectory)
      reject(s"Mandatory directory '$d' not found in bag.")
  }
  ...
  def doesNotContainFile(f: Path)(t: TargetBag): Try[Unit] = Try {
    ...
    if ((t.bagDir / f.toString).exists)
      reject(s"File '$f' MUST NOT exist in bag (of this information package type).")
  }
  ...
  def hasOnlyValidFileNames(t: TargetBag): Try[Unit] = Try {
    ...
    val filesInManifest = t.tryBag.map { bag =>
      bag.getPayLoadManifests.asScala.headOption.getOrElse(reject(s"Dependent rule should have failed: no manifest found for ${ t.bagDir }"))
    }.getOrElse(reject(s"Dependent rule should have failed: Could not get bag ${ t.bagDir }"))
    ...
    if (invalidFiles.nonEmpty)
      reject(invalidFiles.mkString("Payload files must have valid characters. Invalid ones: ", ", ", ""))
  }
  ...
  def isOriginalFilepathsFileComplete(t: TargetBag): Try[Unit] = {
    ...
    t.tryFilesXml.map { xml =>
      ...
      if (originalFileSetsEqual && physicalFileSetsEqual) ()
        ...
        val msg = msg1 + msg2
        reject(s"$originalFilepathsFile errors:\n$msg")
      }
    }
  }
}
