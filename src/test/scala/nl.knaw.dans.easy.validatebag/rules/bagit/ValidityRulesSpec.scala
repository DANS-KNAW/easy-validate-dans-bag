...
class ValidityRulesSpec extends TestSupportFixture {
  ...
  "bagIsValid" should "fail if bag dir does not exist" in {
    testRuleViolation(bagIsValid, "XXX", "does not exist")
  }

  it should "fail if bagit.txt is missing" in {
    testRuleViolation(bagIsValid, "bagit-missing-bagittxt", "'bagit.txt' is missing")
  }

  it should "fail if there is a payload file that is not listed in any of the manifests" in {
    testRuleViolation(bagIsValid, "bagit-unchecksummed-payload-file", "isn't listed in any manifest")
  }

  it should "fail if the checksum of a tag file is incorrect" in {
    testRuleViolationRegex(bagIsValid, "bagit-incorrect-tagfile-checksum", "SHA-1.*but was computed".r)
  }
}
